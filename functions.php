 ...
 */
function rosenberg_disable_cache() {
    // Отключаем кеширование для всех страниц
    header('Cache-Control: no-cache, no-store, must-revalidate, max-age=0');
    header('Pragma: no-cache');
    header('Expires: 0');
}
add_action('init', 'rosenberg_disable_cache');


/**
 * Add version to CSS/JS files to prevent caching
 */
function rosenberg_asset_version($src) {
...
add_filter('style_loader_src', 'rosenberg_asset_version', 10, 1);
add_filter('script_loader_src', 'rosenberg_asset_version', 10, 1);
